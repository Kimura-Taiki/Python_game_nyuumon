...
1 : ゲーム開始の入力を待つ。
    画面をクリックしたら、最初に落ちてくるネコをセットしてindex2の処理に移る。
2 : 【ゲーム中の処理１】
    ネコを落下させる。
    全てのネコが落下したらindex3の処理に移る。
3 : 【ゲーム中の処理２】
    ネコが３つ以上並んだかを調べ、揃った猫があれば肉球に変える。
    index4の処理に移る。
4 : 【ゲーム中の処理３】
    肉球に変わったマスがあれば、消してスコアを加算し、再びindex2の落下処理に
    移る(消えたマスの上にあるネコを落とすため)。
    肉球に変わったマスがない場合、最上段まで積み上がっていなければindex5の
    入力待ちに移る。積み上がってしまったら、index6のゲームオーバーに移る。
5 : 【ゲーム中の処理４】
    プレイヤーからの入力を待つ。
    マウスでカーソルを移動し、クリックしてネコを配置したらindex2のネコを
    落下させる。
6 : ゲームオーバー画面。変数で時間をカウントし、５秒後にindex0の処理に移る。
"""